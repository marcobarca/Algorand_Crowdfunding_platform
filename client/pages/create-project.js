...
import API from '../API';
import { signTxn } from "../utils/ContractOperations";
...
export default function CreateProject() {
    ...
    const handleSubmit = async (event) => {
        ...
        //API
        API.createApp(data.name, token, port, accountContext, parseInt(data.goal), durationInSeconds)
        //API.createAppFromTxn(data.name, accountContext, handleTxn)
        //Now txn state is the transaction builded from the backend. It must be signed and
        //sent back to the backend in order to be sent to the blockchain
        signTxn(txn)
